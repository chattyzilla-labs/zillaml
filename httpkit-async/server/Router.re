...
type routeResponseT = {
  ...
  type routeFn = (Zillaml.Request.t, string) => Deferred.t(option(routeResponseT));

  let createResponse = (~status, ~headers=?, body) => Deferred.return(Some({
    status,
    headers,
    body,
  }))
  ...
  let create_router = routerFn => (req: Zillaml.Request.t, body_) => {
    ...
    let path = Uri.of_string(req.target) |> Uri.path |> String.split_on_char('/') |> List.tl;
