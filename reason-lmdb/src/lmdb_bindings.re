...
external init: unit => ((string, int, int, int), array(int)) = "mdbs_init";
[@ocaml.warning "-8"];
let (
  version,
  [|
...
module Flags: Flags with type t = int = {
  type t = int;
  external (+): (t, t) => t = "%orint";
  external ( * ): (t, t) => t = "%andint";
  let test = (f, m) => f * m == f;
...
external env_get_flags: env => int = "mdbs_env_get_flags";
...
/* dbi */
type dbi;
let invalid_dbi: dbi = (Obj.magic(- 1): dbi);
module DbiFlags = {
  include Flags;
...
module Block_option = {
  type t(+'a);
  let none: t(_) = (Obj.magic(None): t(_));
...
external dbi_open: (txn, option(string), Flags.t) => dbi = "mdbs_dbi_open";
external dbi_close: (env, dbi) => unit = "mdbs_dbi_close";
external dbi_flags: (txn, dbi) => Flags.t = "mdbs_dbi_flags";
