open Alcotest;
open Lmdb;

let () = {
  let (s, major, minor, patch) = Lmdb.version;
  Printf.printf(
    "Version: %s\nOr: (%i,%i,%i)\nOCaml Version: %s\n%!",
    s,
    major,
    minor,
    patch,
    Sys.ocaml_version,
  );
};

let filename = {
  let rec tmp_filename = (base, suffix, n) => {
    let name = Printf.sprintf("%s.%u%s", base, n, suffix);
    if (Sys.file_exists(name)) {
      tmp_filename(base, suffix, n + 1);
    } else {
      name;
    };
  };

  tmp_filename("lmdb_test", ".db", 0);
};

let env =
  Env.create(
    Rw,
    ~flags=Env.Flags.(no_subdir + no_sync + no_mem_init),
    ~map_size=524288,
    ~max_maps=10,
    filename,
  );
let () =
  at_exit @@
  (
    () => {
      Env.close(env);
      Sys.remove(filename);
      Sys.remove(filename ++ "-lock");
    }
  );

[@warning "-26-27"]
let capabilities = () => {
  let map =
    Map.(
      create(
        Nodup,
        ~key=Conv.int32_be_as_int,
        ~value=Conv.int32_be_as_int,
        ~name="Capabilities",
      )
    )(
      env,
    );

  let env_rw = env;
  let env_ro = env;
  /* let env_rw = (env_ro :> [ `Read | `Write ] Env.t) in <- FAILS */
  /* ignore @@ (rw :> [ `Read ] Cap.t); <- FAILS */
  /* ignore @@ (ro :> [ `Read | `Write ] cap); <- FAILS */
  ignore @@
  Txn.go(Rw, env_rw, ~txn=?None) @@
  (
    txn_rw => {
      let txn_ro = (txn_rw :> Txn.t([ | `Read]));
      Map.add(~txn=txn_rw, map, 4, 4);
      /* Map.add ~txn:txn_ro map 4 4; <- FAILS */
      assert(Map.get(~txn=txn_rw, map, 4) == 4);
      assert(Map.get(~txn=txn_ro, map, 4) == 4);
      Cursor.go(Ro, map, ~txn=(txn_rw :> Txn.t([ | `Read]))) @@
      (cursor => assert(Cursor.get(cursor, 4) == 4));
    }
  );
};
/* Cursor.first_dup cursor; <- FAILS */

let check_kv = check(pair(int, int));

let test_types = {
  let map =
    Map.(
      create(
        Nodup,
        ~key=Conv.int32_be_as_int,
        ~value=Conv.int32_be_as_int,
        ~name="Capabilities",
      )
    )(
      env,
    );

  (
    "types",
    [
      (
        "value restriction",
        `Quick,
        () =>
          ignore @@
          Txn.go(Rw, ~txn=?None, env) @@
          (
            txn => {
              Map.stat(~txn, map) |> ignore;
              Map.add(~txn, map, 1, 1);
            }
          ),
      ),
      (
        "can read from writable",
        `Quick,
        () =>
          ignore @@
          Txn.go(Rw, env, (txn: Txn.t([> | `Write])) =>
            Map.stat(~txn, map) |> ignore
          ),
      ),
      ("ro txn on rw env", `Quick, () => Txn.go(Ro, env, ignore) |> ignore),
    ],
  );
};

let unimap =
  Map.(
    create(
      Nodup,
      ~key=Conv.int32_be_as_int,
      ~value=Conv.int32_be_as_int,
      ~name="Nodup",
    )
  )(
    env,
  )
and dupmap =
  Map.(
    create(
      Dup,
      ~key=Conv.int32_be_as_int,
      ~value=Conv.int32_be_as_int,
      ~name="Dup",
    )
  )(
    env,
  )
and unimap_filled =
  Map.(
    create(
      Nodup,
      ~key=Conv.int32_be_as_int,
      ~value=Conv.int32_be_as_int,
      ~name="NodupFilled",
    )
  )(
    env,
  )
and dupmap_filled =
  Map.(
    create(
      Dup,
      ~key=Conv.int32_be_as_int,
      ~value=Conv.int32_be_as_int,
      ~name="DupFilled",
    )
  )(
    env,
  );

let () = {
  let rec loop = n =>
    if (n <= 536870912) {
      Map.(add(unimap_filled, ~flags=Flags.append_dup))(n, n);
      loop(n * 2);
    };
  loop(12);

  let rec loop = n =>
    if (n <= 536870912) {
      let rec loop_dup = m =>
        if (m <= 536870912) {
          Map.(add(dupmap_filled, ~flags=Flags.append_dup))(n, m);
          loop_dup(m * 2);
        };
      loop_dup(n);
      loop(n * 2);
    };
  loop(12);
};

let test_map = (
  "map",
  [
    (
      "add uni",
      `Quick,
      () => {
        try(Map.remove(unimap, 4285)) {
        | Not_found => ()
        };
        Map.add(unimap, 4285, 42);
        Map.get(unimap, 4285) |> check(int, "check after add", 42);
        check_raises("key collision", Exists, () =>
          Map.add(unimap, 4285, 42)
        );
      },
    ),
    (
      "add dup",
      `Quick,
      () => {
        try(Map.remove(unimap, 4285)) {
        | Not_found => ()
        };
        Map.add(dupmap, 4285, 42);
        Map.add(dupmap, 4285, 43);
        Map.get(dupmap, 4285) |> check(int, "check after add", 42);
        check_raises("key collision", Exists, () =>
          Map.(add(~flags=Flags.no_overwrite))(dupmap, 4285, 45)
        );
        check_raises("value collision", Exists, () =>
          Map.(add(~flags=Flags.no_dup_data))(dupmap, 4285, 42)
        );
      },
    ),
    (
      "set uni",
      `Quick,
      () => {
        Map.set(unimap, 4285, 1);
        Map.set(unimap, 4285, 2);
        Map.get(unimap, 4285) |> check(int, "set overwrites", 2);
      },
    ),
    (
      "set dup",
      `Quick,
      () => {
        Map.set(dupmap, 4285, 1);
        Map.set(dupmap, 4285, 2);
        Map.get(dupmap, 4285) |> check(int, "set overwrites", 2);
      },
    ),
    (
      "get uni",
      `Quick,
      () => {
        Map.set(unimap, 4285, 1);
        Map.get(unimap, 4285) |> check(int, "correct read", 1);
      },
    ),
    (
      "get dup",
      `Quick,
      () => {
        Map.set(dupmap, 4285, 1);
        Map.add(dupmap, 4285, 2);
        Map.get(dupmap, 4285) |> check(int, "get returns first value", 1);
      },
    ),
    (
      "remove",
      `Quick,
      () => {
        Map.set(unimap, 4285, 1);
        Map.remove(unimap, 4285);
        check_raises("removed", Not_found, () =>
          Map.get(unimap, 4285) |> ignore
        );
      },
    ),
  ],
);

let test_cursor = (
  "cursor",
  [
    (
      "fold_left uni",
      `Quick,
      () =>
        Cursor.fold_left(
          12,
          unimap_filled,
          ~f=(n, key, value) => {
            check(int, "key", n, key);
            check(int, "values", n, value);
            n * 2;
          },
        )
        |> check(int, "last_key", 805306368),
    ),
    (
      "fold_right uni",
      `Quick,
      () =>
        Cursor.fold_right(
          unimap_filled,
          402653184,
          ~f=(key, value, n) => {
            check(int, "key", n, key);
            check(int, "values", n, value);
            n / 2;
          },
        )
        |> check(int, "last_key", 6),
    ),
    (
      "iter uni",
      `Quick,
      () => {
        let n = ref(12);
        Cursor.iter(
          unimap_filled,
          ~f=(key, value) => {
            check(int, "key", n^, key);
            check(int, "values", n^, value);
            n := value * 2;
          },
        );
        check(int, "last_kv", 805306368, n^);
      },
    ),
    (
      "fold_left dup",
      `Quick,
      () =>
        Cursor.fold_left(
          (12, 12),
          dupmap_filled,
          ~f=((n, m), key, value) => {
            check_kv("kv pair", (n, m), (key, value));
            if (m * 2 <= 536870912) {
              (n, m * 2);
            } else {
              (n * 2, n * 2);
            };
          },
        )
        |> fst
        |> check(int, "last_key", 805306368),
    ),
    (
      "fold_right dup",
      `Quick,
      () =>
        Cursor.fold_right(
          dupmap_filled,
          (402653184, 402653184),
          ~f=(key, value, (n, m)) => {
            check_kv("kv pair", (n, m), (key, value));
            if (m > n) {
              (n, m / 2);
            } else {
              (n / 2, 402653184);
            };
          },
        )
        |> fst
        |> check(int, "last_key", 6),
    ),
    (
      "iter dup",
      `Quick,
      () => {
        let kv = ref((12, 12));
        Cursor.iter(
          dupmap_filled,
          ~f=(key, value) => {
            check_kv("kv pair", kv^, (key, value));
            Printf.eprintf("%u %u\n", key, value);
            if (value * 2 <= 536870912) {
              kv := (key, value * 2);
            } else {
              kv := (key * 2, key * 2);
            };
          },
        );
        check_kv("last_kv", (805306368, 805306368), kv^);
      },
    ),
    (
      "fold_left_all",
      `Quick,
      () =>
        Cursor.fold_left_all(
          12,
          dupmap_filled,
          ~f=(n, key, values) => {
            check(int, "key", n, key);
            let rec loop_dup = (i, m) =>
              if (m <= 536870912) {
                check(int, "dup", m, values[i]);
                loop_dup(i + 1, m * 2);
              } else {
                check(int, "no extra dups", i, Array.length(values));
              };
            loop_dup(0, key);
            key * 2;
          },
        )
        |> check(int, "last_key", 805306368),
    ),
    (
      "fold_right_all",
      `Quick,
      () =>
        Cursor.fold_right_all(
          dupmap_filled,
          402653184,
          ~f=(key, values, n) => {
            check(int, "key", n, key);
            let rec loop_dup = (i, m) =>
              if (m <= 536870912) {
                check(int, "dup", m, values[i]);
                loop_dup(i + 1, m * 2);
              } else {
                check(int, "no extra dups", i, Array.length(values));
              };
            loop_dup(0, key);
            key / 2;
          },
        )
        |> check(int, "last_key", 6),
    ),
    (
      "iter_all",
      `Quick,
      () => {
        let n = ref(12);
        Cursor.iter_all(
          dupmap_filled,
          ~f=(key, values) => {
            check(int, "key", n^, key);
            let rec loop_dup = (i, m) =>
              if (m <= 536870912) {
                check(int, "dup", m, values[i]);
                loop_dup(i + 1, m * 2);
              } else {
                check(int, "no extra dups", i, Array.length(values));
              };
            loop_dup(0, key);
            n := key * 2;
          },
        );
        check(int, "last_key", 805306368, n^);
      },
    ),
    (
      "add uni",
      `Quick,
      () => {
        try(Map.remove(unimap, 4285)) {
        | Not_found => ()
        };
        Cursor.go(Rw, unimap) @@
        (
          cursor => {
            Cursor.add(cursor, 4285, 42);
            Cursor.get(cursor, 4285) |> check(int, "check after set", 42);
            check_raises("key collision", Exists, () =>
              Cursor.add(cursor, 4285, 42)
            );
          }
        );
      },
    ),
    (
      "add dup",
      `Quick,
      () => {
        try(Map.remove(dupmap, 4285)) {
        | Not_found => ()
        };
        Cursor.go(Rw, dupmap) @@
        (
          cursor => {
            Cursor.add(cursor, 4285, 42);
            Cursor.add(cursor, 4285, 43);
            Cursor.get(cursor, 4285) |> check(int, "check after add", 42);
            check_raises("key collision", Exists, () =>
              Cursor.(add(~flags=Flags.no_overwrite))(cursor, 4285, 45)
            );
            check_raises("value collision", Exists, () =>
              Cursor.(add(~flags=Flags.no_dup_data))(cursor, 4285, 42)
            );
          }
        );
      },
    ),
    (
      "set uni",
      `Quick,
      () =>
        Cursor.go(Rw, unimap) @@
        (
          cursor => {
            Cursor.set(cursor, 4285, 1);
            Cursor.set(cursor, 4285, 2);
            Cursor.get(cursor, 4285) |> check(int, "set overwrites", 2);
          }
        ),
    ),
    (
      "set dup",
      `Quick,
      () =>
        Cursor.go(Rw, dupmap) @@
        (
          cursor => {
            Cursor.set(cursor, 4285, 1);
            Cursor.set(cursor, 4285, 2);
            Cursor.get(cursor, 4285) |> check(int, "set overwrites", 2);
          }
        ),
    ),
    (
      "get uni",
      `Quick,
      () =>
        Cursor.go(Rw, unimap) @@
        (
          cursor => {
            Cursor.set(cursor, 4285, 1);
            Cursor.get(cursor, 4285) |> check(int, "correct read", 1);
          }
        ),
    ),
    (
      "get dup",
      `Quick,
      () =>
        Cursor.go(Rw, dupmap) @@
        (
          cursor => {
            Cursor.set(cursor, 4285, 1);
            Cursor.add(cursor, 4285, 2);
            Cursor.get(cursor, 4285)
            |> check(int, "get returns first value", 1);
          }
        ),
    ),
    (
      "remove",
      `Quick,
      () =>
        Cursor.go(Rw, unimap) @@
        (
          cursor => {
            Cursor.set(cursor, 4285, 1);
            Cursor.remove(cursor);
            check_raises("removed", Not_found, () =>
              Cursor.get(cursor, 4285) |> ignore
            );
          }
        ),
    ),
    (
      "walk uni",
      `Quick,
      () =>
        Cursor.(
          go(Rw, unimap, ~txn=?None) @@
          (
            cursor => {
              for (i in 0 to 9) {
                Cursor.add(cursor, i, i);
              };
              first(cursor) |> check_kv("first", (0, 0));
              check_raises("walk before first", Not_found, () =>
                prev(cursor) |> ignore
              );
              next_nodup(cursor) |> check_kv("next_nodup", (1, 1));
              seek(cursor, 5) |> check_kv("seek 5", (5, 5));
              prev(cursor) |> check_kv("prev", (4, 4));
              current(cursor) |> check_kv("current", (4, 4));
              remove(cursor);
              current(cursor) |> check_kv("shift after remove", (5, 5));
              next_nodup(cursor) |> check_kv("next_nodup", (6, 6));
              replace(cursor, 4); /* delete (6,6), add (6,4) */
              current(cursor) |> check_kv("replace", (6, 4));
              last(cursor) |> ignore;
              check_raises("walking beyond last key", Not_found, () =>
                next(cursor) |> ignore
              );
            }
          )
        ),
    ),
    (
      "walk dup",
      `Quick,
      () =>
        Cursor.(
          go(Rw, dupmap, ~txn=?None) @@
          (
            cursor => {
              for (i in 0 to 9) {
                add(cursor, i, i);
              };
              for (i in 0 to 9) {
                add(cursor, 10, i);
              };
              prev(cursor) |> check_kv("prev", (10, 8));
              next(cursor) |> check_kv("next", (10, 9));
              first_dup(cursor) |> check(int, "first_dup", 0);
              next_dup(cursor) |> check(int, "next_dup", 1);
              seek_dup(cursor, 10, 5);
              current(cursor) |> check_kv("seek 5", (10, 5));
              prev(cursor) |> check_kv("prev", (10, 4));
              current(cursor) |> check_kv("current", (10, 4));
              remove(cursor);
              current(cursor)
              |> check_kv("cursor moved forward after remove", (10, 5));
              first_dup(cursor) |> check(int, "first", 0);
              check_raises("fail when walking before first dup", Not_found, () =>
                prev_dup(cursor) |> ignore
              );
              last_dup(cursor) |> check(int, "last", 9);
              check_raises("fail when walking beyond last dup", Not_found, () =>
                next_dup(cursor) |> ignore
              );
              seek_dup(cursor, 10, 7);
              current(cursor) |> check_kv("seek_dup", (10, 7));
            }
          )
        ),
    ),
    (
      "first/last get first/last values",
      `Quick,
      () =>
        Cursor.(
          go(Rw, dupmap, ~txn=?None) @@
          (
            cursor => {
              set(cursor, ~flags=Flags.(none), 0, 0);
              add(cursor, ~flags=Flags.(append_dup), 0, 1);
              add(cursor, ~flags=Flags.(append_dup), 0, 2);
              add(cursor, ~flags=Flags.(append + append_dup), 536870913, 5);
              add(cursor, ~flags=Flags.(append_dup), 536870913, 6);
              add(cursor, ~flags=Flags.(append_dup), 536870913, 7);
              first(cursor) |> check_kv("first value", (0, 0));
              last(cursor) |> check_kv("last value", (536870913, 7));
            }
          )
        ),
    ),
    (
      "*_all",
      `Quick,
      () =>
        Cursor.(
          go(Rw, dupmap_filled, ~txn=?None) @@
          (
            cursor => {
              get_all(cursor, 100663296)
              |> check(
                   array(int),
                   "get_all",
                   [|100663296, 201326592, 402653184|],
                 );
              current(cursor)
              |> check_kv("cursor after get_all", (100663296, 402653184));
              last_all(cursor)
              |> snd
              |> check(array(int), "last_all", [|402653184|]);
              prev_all(cursor)
              |> snd
              |> check(array(int), "prev_all", [|201326592, 402653184|]);
              prev_nodup(cursor) |> ignore;
              next_all(cursor)
              |> snd
              |> check(array(int), "next_all", [|201326592, 402653184|]);
              prev_nodup(cursor) |> ignore;
              next_nodup(cursor) |> ignore;
              current(cursor)
              |> check_kv("cursor after prev_all", (201326592, 201326592));
              current_all(cursor)
              |> snd
              |> check(array(int), "current_all", [|201326592, 402653184|]);
              current(cursor)
              |> check_kv("cursor after current_all", (201326592, 402653184));
              current(cursor)
              |> check_kv("cursor after next_all", (201326592, 402653184));
              first_all(cursor)
              |> snd
              |> check(
                   array(int),
                   "first_all",
                   [|
                     12,
                     24,
                     48,
                     96,
                     192,
                     384,
                     768,
                     1536,
                     3072,
                     6144,
                     12288,
                     24576,
                     49152,
                     98304,
                     196608,
                     393216,
                     786432,
                     1572864,
                     3145728,
                     6291456,
                     12582912,
                     25165824,
                     50331648,
                     100663296,
                     201326592,
                     402653184,
                   |],
                 );
              current(cursor)
              |> check_kv("cursor after first_all", (12, 402653184));
            }
          )
        ),
    ),
    (
      "get_multiple",
      `Quick,
      () => {
        try(Map.remove(unimap, 0)) {
        | Not_found => ()
        };
        Cursor.(
          go(Rw, dupmap, ~txn=?None) @@
          (
            cursor => {
              seek(cursor, 0) |> ignore;
              remove(cursor, ~all=true);
              for (i in 0 to 65536) {
                add(cursor, ~flags=Flags.append_dup, 0, i);
              };
              let values = get_all(cursor, 0);
              for (i in 0 to 65536) {
                if (i != values[i]) {
                  check(
                    int,
                    "order in many dups got with get_all",
                    i,
                    values[i],
                  );
                };
              };
            }
          )
        );
      },
    ),
    (
      "wrong map",
      `Quick,
      () => {
        let map2 =
          Map.(
            create(
              Nodup,
              ~key=Conv.int32_be_as_int,
              ~value=Conv.int32_be_as_int,
              ~name="wrongmap",
            )
          )(
            env,
          );

        check_raises(
          "wrong cursor",
          Invalid_argument("Lmdb.Cursor.fold: Got cursor for wrong map"),
          () =>
          Cursor.go(Ro, map2) @@
          (
            cursor => Cursor.fold_left(~cursor, (), unimap, ~f=(_, _, _) => ())
          )
        );
      },
    ),
  ],
);

let test_int = {
  let make_test = (name, conv) => (
    name,
    `Quick,
    () => {
      let map = Map.(create(Dup, ~key=conv, ~value=conv, ~name))(env);

      let rec loop = n =>
        if (n < 1073741823) {
          try(Map.(add(~flags=Flags.append, map, n, n))) {
          | Exists => fail("Ordering on keys")
          };
          try(Map.(add(~flags=Flags.append_dup, map, 1, n))) {
          | Exists => fail("Ordering on values")
          };
          loop(n / 3 * 4);
        };
      loop(12);
      Map.drop(~delete=true, map);
    },
  );

  (
    "Int",
    [
      make_test("int32_be", Conv.int32_be_as_int),
      make_test("int32_le", Conv.int32_le_as_int),
      make_test("int64_be", Conv.int64_be_as_int),
      make_test("int64_le", Conv.int64_le_as_int),
    ],
  );
};

let test_stress = (
  "threaded GC stress",
  {
    let stress = (duration, ()) => {
      let map =
        Map.(
          create(
            Nodup,
            ~key=Conv.string,
            ~value=Conv.string,
            ~name="map.string",
          )
        )(
          env,
        );

      let mutex = Mutex.create();
      let errors = ref(0);
      let running = ref(true);
      let n = 100;
      let rec worker = thread_id => {
        let offset = thread_id * n;
        for (i in offset to offset + n - 1) {
          let dig = Digest.string @@ string_of_int(i);
          Map.add(map, dig, dig);
        };
        for (i in offset to offset + n - 1) {
          let dig = Digest.string @@ string_of_int(i);
          if (Map.get(map, dig) != dig) {
            Mutex.lock(mutex);
            incr(errors);
            Mutex.unlock(mutex);
          } else {
            Map.remove(map, dig);
          };
        };
        if (running^) {
          worker(thread_id);
        };
      };

      let rec stress_gc = () => {
        Gc.minor();
        Thread.yield();
        if (running^) {
          stress_gc();
        };
      };

      Gc.full_major();
      Thread.create(stress_gc, ()) |> ignore;
      let threads = Array.init(8, Thread.create(worker));
      Thread.delay(duration);
      running := false;
      Array.iter(Thread.join, threads);
      check(int, "wrong reads", 0, errors^);
      Map.drop(~delete=true, map);
    };

    [
      ("stress 1s", `Quick, stress(1.)),
      /*; "stress 500s", `Slow, stress 500. too slow */
    ];
  },
);

let test_regress = (
  "regression tests",
  [
    (
      "unnamed dbi",
      `Quick,
      () =>
        Map.(open_existing(Nodup, ~key=Conv.string, ~value=Conv.string))(env)
        |> ignore,
    ),
    (
      "dup unnamed dbi",
      `Quick,
      () =>
        check_raises(
          "no duplicates on unnamed map",
          Invalid_argument(
            "Lmdb.Map.create: The unnamed map does not support duplicates",
          ),
        ) @@
        (
          () => {
            let unnamed_dup =
              Map.(open_existing(Dup, ~key=Conv.string, ~value=Conv.string))(
                env,
              );

            check(
              bool,
              "compare dups",
              true,
              Map.compare_val(unnamed_dup, "5", "1") > 0,
            );
            Cursor.go(Rw, unnamed_dup) @@
            (
              cursor => {
                Cursor.add(cursor, "dup_entry", "1");
                Cursor.add(cursor, "dup_entry", "2");
                check(
                  pair(string, array(string)),
                  "dup entries",
                  ("dup entry", [|"1", "2"|]),
                  Cursor.current_all(cursor),
                );
              }
            );
          }
        ),
    ),
  ],
);

let test_txn = (
  "transaction",
  {
    let map =
      Map.(create(Nodup, ~key=Conv.int32_le_as_int, ~value=Conv.string))(
        env,
        ~name="double txn_abort",
      );
    [
      (
        "abort",
        `Quick,
        () => {
          ignore @@
          Txn.go(
            Rw,
            env,
            txn => {
              Map.add(~txn, map, 13, "blub");
              Txn.abort(txn);
            },
          );
          check_raises("expecting Not_found", Not_found, () =>
            Map.get(map, 13) |> ignore
          );
        },
      ),
      (
        "wrong envirronment",
        `Quick,
        () => {
          let env2 =
            Env.create(
              Ro,
              ~flags=Env.Flags.(no_subdir + no_sync + no_lock + no_mem_init),
              filename,
            );

          check_raises(
            "wrong txn",
            Invalid_argument("Lmdb: transaction from wrong environment."),
            () =>
            ignore @@ Txn.go(Ro, env2, txn => Map.get(~txn, map, 0) |> ignore)
          );
          Env.close(env2);
        },
      ),
      (
        "MAP_FULL triggered by txn_commit",
        `Quick,
        () => {
          /* mdb_txn_commit may return MDB_MAP_FULL.
           * In that case we did call mdb_txn_abort, which resulted in the
           * transaction being freed twice. */
          check_raises("expecting Map_full from txn_commit", Map_full, () =>
            for (i in 100 to max_int)
              {
                Map.(add(~flags=Flags.append))(map, i, "blub");
              } /* Calls Txn.trivial */
          );
          let map_size = Env.(info(env)).map_size;
          Env.set_map_size(env, 2 * map_size);
          check(pass, "resized map", (), ());
          Map.add(map, 1, "blub");
          check(pass, "add successfull", (), ());
          Map.drop(~delete=false, map);
        },
      ),
      (
        "MAP_FULL triggered by Map.add",
        `Quick,
        () => {
          check_raises("expecting Map_full from Txn.go", Map_full, () =>
            ignore @@
            Txn.go(Rw, env) @@
            (
              txn => {
                let bulk = String.make(1024, '#');
                for (i in 100 to max_int) {
                  Map.(add(~txn, ~flags=Flags.append))(map, i, bulk);
                };
              }
            )
          );
          Map.add(map, 1, "blub");
          check(pass, "add successfull", (), ());
          Map.drop(~delete=false, map);
        },
      ),
      (
        "MAP_FULL not passed on to Txn.go",
        `Quick,
        () => {
          check_raises(
            "expecting MDB_BAD_TXN from Txn.go", Error(- 30782), () =>
            ignore @@
            Txn.go(Rw, env) @@
            (
              txn => {
                let bulk = String.make(1024, '#');
                check_raises("expecting Map_full from Map.add", Map_full) @@
                (
                  () =>
                    for (i in 100 to max_int) {
                      Map.(add(~txn, ~flags=Flags.append))(map, i, bulk);
                    }
                );
              }
            )
          );
          Map.add(map, 1, "blub");
          check(pass, "add successfull", (), ());
        },
      ),
    ];
  },
);

let () =
  run(
    "Lmdb",
    [
      ("capabilities", [("capabilities", `Quick, capabilities)]),
      test_types,
      test_map,
      test_cursor,
      test_int,
      test_regress,
      test_txn,
      test_stress,
      Pr.test(env),
    ],
  );
