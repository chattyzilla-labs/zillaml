...
let (): unit = {
  switch (last([ "a" , "b" , "c" ,"d" ])) {
  | Some(a) => print_endline(Printf.sprintf("last([\"a\", \"b\", \"c\", \"d\"]) = Some(\"%s\")\n", a))
  | None => print_endline("None")
  };
};

let rec last_two = fun
  | [] | [_] => None
  | [a, b] => Some((a, b))
...
let () = {
  ...
  | Some((a, b)) when a == "c" && b == "d" => print_endline(Printf.sprintf("last_two([\"a\", \"b\", \"c\", \"d\"]) = Some((\"%s\", \"%s\"))\n", a, b))
  | _ => raise(OUTPUT_ERROR("Unexpected value returned for last_two test"))
  };
};

type node('a) = | One('a) | Many(list(node('a)));
...
let () = {
  switch (flatten([One("a"), Many([One("b"), Many([One("c"), One("d")]), One("e")])])) {
  | ["a", "b", "c", "d", "e"] =>
    print_endline(
...
let () = {
  switch (flatten_with_aux([One("a"), Many([One("b"), Many([One("c"), One("d")]), One("e")])])) {
  | ["a", "b", "c", "d", "e"] =>
    print_endline(
      {|flatten_with_aux([One("a"), Many([One("b"), Many([One("c"), One("d")]), One("e")])]) = ["a", "b", "c", "d", "e"]|}
    )
    print_newline();
  | _ => raise(OUTPUT_ERROR("Unexpected value returned for flatten_with_aux test"))
  };
};

// Eliminate consecutive duplicates of list elements
let rec compress = fun
...
let () = {
  switch (
    compress(["a", "a", "a", "b", "c", "c", "c", "a", "a", "d", "e", "e", "e"])
  ) {
  | ["a", "b", "c", "a", "d", "e"] =>
    print_endline(
  ...
  | _ =>
    raise(
      OUTPUT_ERROR("Unexpected value returned for compress test"),
    )
  };
};
...
let () = {
  switch (
    compress_with_aux(["a", "a", "a", "b", "c", "c", "c", "a", "a", "d", "e", "e", "e"])
  ) {
  | ["a", "b", "c", "a", "d", "e"] =>
    print_endline(
...
let pack = list => {
  let rec aux = (sublist, acc) => fun
    | [] => []
    | [a] => [[a, ...sublist], ...acc]
    | [a, ...([b, ..._] as t)] => a == b ? aux([a, ...sublist], acc, t) : aux([], [[a, ...sublist], ...acc], t)
    ;
  List.rev(aux([], [], list));
}
...
let () = {
  ...
  | _ =>
    raise(
      OUTPUT_ERROR("Unexpected value returned for pack test"),
    )
  };
};
...
let () = {
  ...
  | _ =>
    raise(
      OUTPUT_ERROR("Unexpected value returned for rotate test given negative int"),
    )
  };
};
...
let () = {
  ...
  | _ =>
    raise(
      OUTPUT_ERROR("Unexpected value returned for rotate test given positive int"),
    )
  };
};

let rec extract_head = (set, n) => fun
  | k when k == n => [set]
  | k when n - k < 0 => []
  | 1 => switch set {
    | [] => []
...
let rec extract_non_stack_safe  = (k, list) =>
  if (k <= 0) {
    ...
    | [] => []
    | [h, ...tl] =>
      let with_h = List.map(l => [h, ...l], extract_non_stack_safe(k - 1, tl));
