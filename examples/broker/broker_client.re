...
let shutdown =
  Common.with_rpc_conn(conn => Rpc.Rpc.dispatch_exn(shutdown_rpc, conn, ()));
...
let subscribe = (~topic) =>
  Common.with_rpc_conn(conn => {
    let clear_string = "\027[H\027[2J";
    switch%bind (Rpc.Pipe_rpc.dispatch(subscribe_rpc, conn, topic)) {
    | Error(err) => Error.raise(err)
    | Ok(Error(s)) =>
      eprintf("subscribe failed: %s\n", s);
      return();
    | Ok([@implicit_arity] Ok(pipe, _id)) =>
      Pipe.iter(
        pipe,
        ~f=msg => {
...
let columns = [
  col("topic", d => Topic.to_string(d.Dump.message.Message.topic)),
  col("text", d => d.Dump.message.Message.text, ~max_width=25),
  col("#sub", d => Int.to_string(d.Dump.num_subscribers)),
  col("time", d =>
    Time.to_sec_string(
      ~zone=force(Time.Zone.local),
      d.Dump.message.Message.time,
    )
  ),
];
...
let dump = (~sexp) =>
  Common.with_rpc_conn(conn => {
    let%bind dump = Rpc.Rpc.dispatch_exn(dump_rpc, conn, ());
    if (sexp) {
      sexp_print_dump(dump);
    } else {
      table_print_dump(dump);
    };
    return();
  });
...
let clear = topic =>
  Common.with_rpc_conn(conn => Rpc.Rpc.dispatch_exn(clear_rpc, conn, topic));
