...
module Make = (Message: Message) => {
  ...
  let subscribe_dir = (root, topic, _writer) => {
...
module Make_v2 = (Message: Message) => {
  module Topic = {
    module Directory = {
      [@deriving (sexp, hash, bin_io, compare, yojson)]
      type t =
  ...
  let subscribe_dir = (root, topic, writer) => {
    open Rpc.Pipe_rpc.Direct_stream_writer;
    let rec aux = (topic, node) => {
      switch (topic) {
      | [] =>
        ...
        switch (node.last_message) {
        | None => ()
        | Some(msg) => let _value = Rpc.Pipe_rpc.Direct_stream_writer.write_without_pushback(writer, msg)
        };
      | [hd, ...tl] =>
        let next_node =
          Hashtbl.find_or_add(
